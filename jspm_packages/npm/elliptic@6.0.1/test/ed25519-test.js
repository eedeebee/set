...
(function(process) {
  ...
  describe('ed25519 derivations', function() {
    ...
    function testFactory(i) {
      it('can compute correct a and A for secret: ' + i, function() {
        ...
        var xRecovered = toHex(ed25519.encodeInt(ed25519.decodePoint(key.pubBytes()).getX()));
  ...
  describe('sign.input ed25519 test vectors', function() {
    ...
    before(function(done) {
      ...
      fs.readFile(__dirname + '/fixtures/sign.input', function(err, f) {
        ...
        assert.equal(lines.length, expectedTests + 1);
        done();
      });
    });
    function testFactory(i) {
      it('vector ' + i, function() {
        ...
        var forged = msg.length === 0 ? [0x78] : msg.slice(0, msg.length - 1).concat((msg[(msg.length - 1)] + 1) % 256);
  ...
  describe('EDDSA(\'ed25519\')', function() {
    ...
    describe('KeyPair', function() {
      var pair;
      var secret = '00000000000000000000000000000000' + '00000000000000000000000000000000';
      before(function() {
      ...
      it('#getPub returns bytes with optional encoding', function() {
        ...
        assert.equal(pair.getPublic('hex'), '3b6a27bcceb6a42d62a3a8d02a6f0d73653215771de243a63ac048a18b59da29');
      });
    });
  });
})(require("process"));
