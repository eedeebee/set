...
export default class Game {
    ...
    constructor($scope) {
        ...
        this.fb = app.fb.child("game-"+this.id);

        // this.user = angularfire.$firebaseObject(app.fb.child("users").child(this.auth.uid));
        //var u = app.fb.child("users").child(this.auth.uid);

        var game = this;
        app.fb.once('value', function(snap) {
            if (!snap.exists()) {
                game.redeal();
            }
        }, function(err) {
        ...
        app.fb.child('users').on('value', function(snap) {
            if (snap.exists()) {
    ...
    update(game) {
        ...
        if (game.set != undefined) {
            ...
            if (game.last_uid != this.auth.uid) {
